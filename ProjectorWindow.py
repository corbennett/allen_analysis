...
class ProjectorWindow(visual.Window):
    '''
    ...
    def __init__(self, projectorType=Projector.Normal, warp=Warp.Disabled, warpfile=None, warpGridsize=300, eyepoint=(0.5, 0.5),
                warpWarp=False, warpShift=[0,0,0,0], warpAngle=[0,0], flipHorizontal=False, flipVertical=False, *args,**kwargs):
        self.projectorType = projectorType
        self.warp = warp
        self.warpfile = warpfile
        
        self.warpWarp = warpWarp
        self.warpShift = warpShift
        self.warpAngle = warpAngle
    ...
    def projectionWarpfile (self):
        ...
        try:
            ...
            if self.warpWarp:
                warpWarp(warpdata,rows,cols,self.warpShift,self.warpAngle)
            
            self.warpMeshRes = [cols, rows]
        except:
...
#        GL.glEnable( GL.GL_POLYGON_SMOOTH ) ; #corbett added
#        GL.glHint( GL.GL_POLYGON_SMOOTH_HINT, GL.GL_NICEST ) ; #corbett added
...
#                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_LINEAR)    #corbett modified
#                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_LINEAR)
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE)
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE)
...
#        self.viewScale = [1, 0.5]
        if self.viewScale is not None:
            GL.glMatrixMode(GL.GL_PROJECTION)
            GL.glLoadIdentity()
#            GL.glOrtho(-self.aspect, self.aspect, -1, 1, -1, 1)
...
def warpWarp(warpData,rows,cols,warpShift,warpAngle):
    
    # rescale x from -1 to 1
    warpData[:,0] = np.tile(np.linspace(-1,1,cols),rows)

    # downsample each row of texture coordinates (u,v) and intensity values by 2 and duplicate on left and right sides
    for i in xrange(rows):
        for j in [2,3,4]:
            warpData[i*cols:(i+1)*cols,j] = np.tile(warpData[i*cols:(i+1)*cols,j][0::2],2)
    
    # rescale texture horizontal coordinate (u) to 0-0.5 (left) and 0.5-1 (right)
    warpData[:,2] /= 2
    warpData[warpData[:,0]>=0,2] += 0.5

    # horizontally stretch texture (u)
    warpData[:,2] /= 2
    warpData[:, 2] += 0.25

    # shift texture coordinates (u,v) and intensity values
    if any(warpShift):
        for j in [2,3,4]:
                w = warpData[:,j].reshape((rows,cols))
                
                if warpShift[0]>0:
                    shiftWarp(w[:,:cols/2].T,warpShift[0])
                elif warpShift[0]<0:
                    shiftWarp(w[:,cols/2::-1].T,-warpShift[0])

                if warpShift[1]>0:
                    shiftWarp(w[:,:cols/2],warpShift[1])
                elif warpShift[1]<0:
                    shiftWarp(w[::-1,:cols/2],-warpShift[1])
                    
                if warpShift[2]>0:
                    shiftWarp(w[:,cols/2:].T,warpShift[2])
                elif warpShift[2]<0:
                    shiftWarp(w[::-1,:cols/2-1:-1].T,-warpShift[2])
                    
                if warpShift[3]>0:
                    shiftWarp(w[:,cols/2:],warpShift[3])
                elif warpShift[3]<0:
                    shiftWarp(w[::-1,cols/2:],-warpShift[3])
                    
                warpData[:,j] = w.reshape((1,-1))
    
    # rotate texture coordinates (u,v)
    if warpAngle[0]!=0: # rotate left side
        rotateWarp(warpData,warpAngle[0],np.where(warpData[:,0]<0)[0])
    
    if warpAngle[1]!=0: # rotate right side
        rotateWarp(warpData,warpAngle[1],np.where(warpData[:,0]>=0)[0])
    
def shiftWarp(w,shift):
    # shift down (input flipped and/or transposed as necessary for correct axis and direction)
    w[shift:,:] = w[:-shift,:]
    w[:shift,:] = w[shift,:]

def rotateWarp(warpData,angle,rowInd):
    angle *= np.pi/180
    rotationMat = np.array([[np.cos(angle),np.sin(angle)],
                            [-np.sin(angle),np.cos(angle)]])
    warpData[rowInd,2:4] = np.dot(warpData[rowInd,2:4],rotationMat)

# center edge smoothing
#    sigma = 1.5
#    xconv = np.arange(-3*sigma, 3*sigma + 1)
#    gaussian = np.exp(-(xconv/sigma)**2/2)
#    warpWarpData[:, 4] = np.convolve(warpWarpData[:, 4], gaussian, 'same')
#    warpWarpData[np.logical_and(np.logical_and(warpWarpData[:, 2] >0, warpWarpData[:, 3] > 0), warpWarpData[:, 4] ==0), 4] = 0.1


if __name__ == "__main__":
    win = ProjectorWindow(fullscr=True, projectorType=Projector.Normal, warp=Warp.Cylindrical, monitor='testmonitor')
