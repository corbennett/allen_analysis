...
class popProbeData():
    ...
    def analyzeRF(self):
        ...
        noUseInd = np.logical_or(np.logical_or(onArea<minRFCutoff,onArea>maxRFCutoff),np.logical_or(onAspect<minAspectCutoff,onAspect>maxAspectCutoff))
        onArea[noUseInd] = np.nan
        onAspect[noUseInd] = np.nan
        offArea = np.pi*np.prod(offFit[:,2:4],axis=1)
        offAspect = offFit[:,2]/offFit[:,3]
        noUseInd = np.logical_or(np.logical_or(offArea<minRFCutoff,offArea>maxRFCutoff),np.logical_or(offAspect<minAspectCutoff,offAspect>maxAspectCutoff))
        offArea[noUseInd] = np.nan
        offAspect[noUseInd] = np.nan
        rfAreaCombined = np.nanmean(np.stack((onArea,offArea)),axis=0)
        rfAspectCombined = np.nanmean(np.stack((onAspect,offAspect)),axis=0)
...
#        sizeTuningOff = np.stack(self.data.run.sparseNoise.sizeTuningOff[ind])
        ...
        ccfY = np.array(data.index.get_level_values('ccfY'))
        ccfX = np.array(data.index.get_level_values('ccfX'))    
        ccfZ = np.array(data.index.get_level_values('ccfZ'))
        ...
        # plot RF aspect in and out of SC axons
        for rfa in (rfAspectCombined[inSCAxons],rfAspectCombined[~inSCAxons]):
            plt.figure(facecolor='w')
            ax = plt.subplot(1,1,1)
            ax.hist(rfa[~np.isnan(rfa)],bins=np.arange(0,maxAspectCutoff,0.2),color='k')
            ax.spines['right'].set_visible(False)
            ax.spines['top'].set_visible(False)
            ax.tick_params(direction='out',top=False,right=False,labelsize='large')
...
#        uindex = uindex[np.in1d(uindex,np.where(peakToTrough>=0.35)[0])]
